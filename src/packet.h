...
struct PhysicalPacket
{
	...
	uint8_t data[]; 			// packet data, not include checksum's byte
};

struct DataPacket
{
	uint8_t type;				// include commands and command type, the same with physical
	uint8_t id;					// packet identification
	uint8_t unique_number[4];	// devices identification
	uint8_t data_length;		// the length of data, can be calculated by data type
	uint8_t data_type;			// type of data, or data's format
	uint8_t data[]; 			// data, not include checksum's byte
};
